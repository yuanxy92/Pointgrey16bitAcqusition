        ...
        for (unsigned int imageCnt = 0; imageCnt < k_numImages; imageCnt++)
        {
            try
            {
                ...
                else
                {
					...
					cv::Mat img(3000, 4000, CV_16U);
					// copy to opencv mat
					std::memcpy(img.data, pResultImage->GetData(), sizeof(unsigned short) *
						pResultImage->GetWidth() * pResultImage->GetHeight());

					cv::Mat img2(3000, 4000, CV_16UC3);
					cv::cvtColor(img, img2, cv::COLOR_BayerRG2RGB);
...
int RunSingleCamera(CameraPtr pCam)
{
    ...
    try
    {
		...
		std::string dstPixelFormat = "BayerRG16";
